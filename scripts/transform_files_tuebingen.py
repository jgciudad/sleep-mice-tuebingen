...
def load_kornum_labels(label_path):
    ...
    df = pd.read_csv(join(config.DATA_DIR, label_path), skiprows=9, engine='python', sep='\t', index_col=False).iloc[:, 4]
    ...
    return stages.to_list(), df.index.values.tolist()

def load_raw_recording(label_path):
    """
    ...
    """
    signal_path = join(config.DATA_DIR, label_path.split("/")[0], 'EDF', basename(label_path)[:-4] + '.edf')
...
def read_kornum_recording(label_path: str, dataset: str):
    ...
    features_dict, sample_start_times = load_raw_recording(label_path)
    ...
    with tables.open_file(config.DATA_FILE, mode='w', title='data from Kornum') as f:
        ...
        for dataset in datasets:
            ...
            for i, label_path in enumerate(config.DATA_SPLIT[dataset]):
                ...
                features, labels, times, epoch_number = read_kornum_recording(label_path, dataset)
